...
public class ServerUdpThread extends Thread {
        ...
        try {
            ...
            switch (opCode) {
                case 1: // QueryAvailability
                    return handleQueryAvailability(buffer, payloadLen, requestId);
                case 2: // Book
                    return handleBook(buffer, payloadLen, requestId, clientInfo);
                case 3: // Change
                    return handleChange(buffer, payloadLen, requestId, clientInfo);
                case 4: // Monitor
                    return handleMonitor(buffer, payloadLen, requestId, clientInfo);
                case 5: // Cancel
                    return handleCancel(buffer, payloadLen, requestId, clientInfo);
                case 6: // CheckIn
                    return handleCheckIn(buffer, payloadLen, requestId, clientInfo);
                default:
                    return createErrorResponse(3, "Unknown operation");
            }
        } catch (Exception e) {
    ...
    private byte[] handleQueryAvailability(ByteBuffer buffer, int payloadLen, int requestId) {
        try {
            ...
            String facilityName = new String(facilityNameBytes, "UTF-8");
    ...
    private byte[] handleChange(ByteBuffer buffer, int payloadLen, int requestId, ClientInfo clientInfo) {
        try {
            ...
            String confirmId = new String(confirmIdBytes, "UTF-8");
